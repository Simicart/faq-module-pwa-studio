...
import { useArticle } from '@faq/faq/src/talons/useArticle'
...
const Article = props => {
	...
	const onChange = event => {
		...
		if(event.target.value) {
			...
			setFilteredSuggestions(articles.filter(
				article => article.name.toLowerCase().includes(event.target.value.toLowerCase())
			));
		} else {
	...
	const onKeyDown = e => {
	    ...
	    if (e.keyCode === 13) { // enter
    		...
    		window.location.href = `/faq/article/${filteredSuggestions[activeSuggestion].url_key}`;
	    }
	      	...
	      	if (activeSuggestion === filteredSuggestions.slice(0, 3).length - 1) {
	        	return;
	      	} else if (activeSuggestion === filteredSuggestions.slice(0,3).length) {
	    		setActiveSuggestion(0);
	...
	return (
		<div>
			<Helmet>
                <meta charSet="utf-8" />
                <title>{data.name} - Frequently Answer and Question</title>
            </Helmet>
			<SearchBox 
				onChange={onChange} 
				ref={inputEl}
